...
fn min_max(board: &mut Board, _depth: u8, max_player: Player) -> GameResult {
    ...
    match result {
        ...
        GameResult::InProgress => {
            match max_player {
                Player::X => { // We want to maximize the score
                    let mut best_score = GameResult::Player(Player::O);
                    let possible_moves: Vec<usize> = board.fields
                        .iter()
                        .enumerate()
...
impl Game {
    ...
    fn make_rand_move(&mut self) {
        let possible_moves: Vec<usize> = self.board.fields
            .iter()
            .enumerate()
            .filter(|(_, value)| **value == Field::Free)
            .map(|(index, _)| index)
...
fn main() {
    let result1 = GameResult::Player(Player::X);
    let result2 = GameResult::Player(Player::O);

    let comparison = result1.partial_cmp(&result2);
